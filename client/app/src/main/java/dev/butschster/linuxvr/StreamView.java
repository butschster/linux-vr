...
public class StreamView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    // Ports are spaced ten apart, matching the server's internal/monitors. That
    // package is the single source of truth for monitor order; if this and it
    // disagree, a window silently shows one screen while its clicks land on
    // another. The session screen already knows the real port — this is the
    // fallback for a window restored by the shell without one.
